...
% Total Energy
Et(:,1,:) = rho(:,1,:).*( e(:,1,:) + 0.5*(u(:,1,:).^2 + v(:,1,:).^2 + w(:,1,:).^2) );
Et(:,end,:) = rho(:,end,:).*( e(:,end,:) + 0.5*(u(:,end,:).^2 + v(:,end,:).^2 + w(:,end,:).^2) );
Et(:,:,1) = rho(:,:,1).*( e(:,:,1) + 0.5*(u(:,:,1).^2 + v(:,:,1).^2 + w(:,:,1).^2) );
Et(:,:,end) = rho(:,:,end).*( e(:,:,end) + 0.5*(u(:,:,end).^2 + v(:,:,end).^2 + w(:,:,end).^2) );

Et(1,:,:) = rho(1,:,:).*...
            ( e(1,:,:) + 0.5*(u(1,:,:).^2 + v(1,:,:).^ 2+ w(1,:,:).^2) ); % @INLET
Et(end,:,:) = rho(end,:,:).*...
              ( e(end,:,:) + 0.5*(u(end,:,:).^2 + v(end,:,:).^2 + w(end,:,:).^2) ); % @OUTLET
